...
useEffect(() => {
  const getPostData = async () => {
    try {
      const response = await axios.get(`${baseURL}/post/getSinglePost/${id}`, {
        headers: {
      ...
      if (fetchedPost && fetchedPost.user) {
        const postOwnerId = fetchedPost.user?._id || fetchedPost.user;

        if (postOwnerId.toString() !== loggedInUserId?.toString()) {
          Swal.fire({
            icon: "error",
            title: "Unauthorized!",
...
}, [id, baseURL, token, validUser, navigate]);
  ...
  // update function
    ...
    // update api
    try {
      const response = await axios.put(`${baseURL}/post/updateSinglePost/${id}`, formData, {
        headers: {
            ...
            >
              ...
              <div className="flex-1 p-5">
                <div className="form-control sm:text-center">
                  <h3 className="text-xl sm:text-2xl text-red-500 font-bold mb-4">Update Post</h3>
                </div>
                {/* Title Input */}
                <div className="form-control">
                  <label className="label py-1">
                    <span className="label-text font-medium text-sm">Update Title</span>
                  </label>
                  <input type="text" key={postData?.title} defaultValue={postData?.title} placeholder="update your post title" className="input input-bordered w-full" name="title"
                  />
                </div>
                ...
                <div className="form-control">
                  <label className="label py-1">
                    <span className="label-text font-medium text-sm">Post Update Description</span>
                  </label><br></br>
                  <textarea key={postData?.description} defaultValue={postData?.description} className="textarea w-full" placeholder="update your post description" name="description"></textarea>
                </div>
                {/* show post image */}
                <div>
                  <div className="form-control">
                  ...
                  <img alt="User Profile" className="w-full h-40" src={postData?.image === "default-image.png" ? '/default/default-image.png' : postData?.image} />
                </div>
                </div>
                {/* image */}
                <div className="form-control">
                ...
                <div className="form-control mt-6 w-full flex flex-col sm:flex-row gap-2">
                  <button type="submit" disabled={btnLoading} className="btn bg-green-500 border-none font-bold text-white px-4 w-full sm:w-1/2">
                    {btnLoading ? "Post Updated..." : "Update Post"}
                  </button>
