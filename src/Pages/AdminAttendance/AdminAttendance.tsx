...
const AdminAttendance = () => {
  ...
  // get all admin attendance data in api
  const { data: adminAttendanceList = [], isLoading } = useQuery<AttendanceRecord[]>({
    queryKey: ["adminAttendance", token],
    queryFn: async () => {
      const response = await axios.get(`${baseURL}/attendance/getAdminAttendance`, {
        headers: {
  ...
  return (
    <div className="p-5 max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6 border-b-2 border-red-500 pb-3">
        <h3 className="text-md sm:text-lg md:text-xl lg:text-2xl font-bold text-red-500">All Attendance (Admin)</h3>
        <button 
          onClick={() => navigate(-1)} 
        ...
        <div className="w-full overflow-x-auto rounded-lg">
          {isLoading ? (
            ...
            <table className="table w-full min-w-[600px]">
              ...
              <tbody>
                {adminAttendanceList.map((row, index) => {
