...
const AllPaymentManager = () => {
  ...
  // call api all payment manager
  const { data: paymentResponse, isLoading, isError } = useQuery({
    queryKey: ['managerPaymentHistory'],
    queryFn: async () => {
      const response = await axios.get(`${baseURL}/payment/getAllPaymentManager`, {
  ...
  return (
    <div className="w-full bg-white p-6 rounded-md shadow-sm">
      ...
      <div className="flex justify-between items-center mb-6 pb-4 border-b border-green-500">
        <h2 className="text-[11px] sm:text-lg font-bold text-green-600">Office Payment History <span className="px-2 py-1 bg-green-100 text-green-700 font-bold text-xs rounded-full">
                  ...
                  <tr key={payment._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-gray-400 font-normal">{payment.user?.email || 'N/A'}</span>
                      </div>
                    </td>
...
export default AllPaymentManager;